...
#include <string.h>
/*
void paint_line(int x, int y, int length, int dir, int color, u8* screen) {
	y++;
	color=0xFF;
...
//Draw 3d Box. Should move this to draw.c and use drawPixel.
void drawBox(int x, int y, int w, int h, int offset3D,int color, u8* frame) {
	x += offset3D;
	int coord = 720 * x + 720 - (3 * y + 3);
	int* p = (int*) (*frame + coord);
	int* t;
	h = (h * 3) >> 2;
	for (x = 0; x < w; x++) {
		t = p;
